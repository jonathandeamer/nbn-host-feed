...
def lambda_handler(event, context):
    ...
    feed_url = 'https://nbn-host-feed.s3.eu-west-2.amazonaws.com/miranda2.xml'
    ...
    with requests.Session() as s:
        ...
        if page.status_code != 200:
            logger.error(f"Failed to get page: {nbn_host_profile}")
        else:
    ...
    file_path = '/tmp/miranda2.xml'
    fg.rss_file(file_path)

    # Upload the XML file to S3
    bucket = 'nbn-host-feed'
    object_name = 'miranda2.xml'
    s3 = boto3.client('s3')
    s3.upload_file(file_path, bucket, object_name, ExtraArgs={'ContentType': 'application/rss+xml'})

    logger.info("RSS feed generated and uploaded to S3.")

    return {
        'statusCode': 200,
        'body': json.dumps('RSS feed generated and uploaded to S3.')
    }
